...
Cromosoma & Geneticos::AGEUniforme(int tam_poblacion) {
    ...
    while (num_evaluaciones < EVALS){
        ...
        num_cruces = poblacion_nueva.cromosomas.size();
        ...
        // Reevaluamos la población porque vamos a checkear su fitness.
        poblacion_nueva.reevaluar();

        // En el algoritmo estacionario se reemplazan los dos descendientes generados tras los cruces y la mutación,
        // por los dos peores de la población actual, en caso de ser mejores que ellos.

        for (int i = 0; i < poblacion_nueva.cromosomas.size(); ++i){
            if (poblacion.buscarPeorCromosoma().fitness < poblacion_nueva.cromosomas[i].fitness){
                poblacion.buscarPeorCromosoma() = poblacion_nueva.cromosomas[i];
            }
        }
    }
...
Cromosoma & Geneticos::AGEPosicion(int tam_poblacion) {
